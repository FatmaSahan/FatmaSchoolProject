...
Feature: ViceDean should be able to create Lesson Program
  ...
  Scenario: Select Lesson by Choosing Teacher
    Given a User wants to select a lesson
    And the User clicks on the "Choose Lesson" button
    And the User selects a teacher from the "Choose Teacher" dropdown
    And the User clicks on the "Submit" button
    Then the User will see a "Successful" message displayed on the screen.